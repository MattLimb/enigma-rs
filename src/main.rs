...
use config::EnigmaTrait;


fn encrypt(input_string: String, rotors: PathBuf, config: PathBuf) {
    let rotor_config = config::rotors::RotorsConfig::load(rotors);
    let mut enigma_config = config::enigma::EnigmaMachine::load(config);
    enigma_config.init(rotor_config);

    let mut output_string = String::new();

    let banned_chars: [char; 33] = [' ', '!', '"', '#', '$', '%', '&', '\'', '(', ')', '*', '+', ',', '-', '.', '/', ':', ';', '<', '=', '>', '?', '@', '[', '\\', ']', '^', '_', '`', '{', '|', '}', '~'];
...
fn list_rotors(rotors: PathBuf, config: PathBuf, detail: bool) {
    ...
    for (idx, (name, rotor)) in rotor_config.clone().0.iter().enumerate() {
...
fn main() {
    ...
    match cli_args.commands {
        EnigmaCommands::Encrypt { input_string } => encrypt(input_string, cli_args.rotors, cli_args.config),
        EnigmaCommands::Decrypt { input_string } => encrypt(input_string, cli_args.rotors, cli_args.config),
